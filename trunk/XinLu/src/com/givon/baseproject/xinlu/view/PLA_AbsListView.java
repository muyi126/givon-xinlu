 ...
 * subclases of this class can display the content of the list in a grid, in a carousel, as stack, etc.
 * 
...
public abstract class PLA_AbsListView extends PLA_AdapterView<ListAdapter> implements
	...
	/**
	 * The list will automatically scroll to the bottom when a data set change notification is received and only if the last item is already
	 * visible on screen.
	 * 
	 * @see #setTranscriptMode(int)
	 */
	...
	public interface OnScrollListener {
		...
		/**
		 * The view is not scrolling. Note navigating the list using the trackball counts as being in the idle state since these transitions
		 * are not animated.
		 */
		...
		/**
		 * Callback method to be invoked while the list view or grid view is being scrolled. If the view is being scrolled, this method will
		 * be called before the next frame of the scroll is rendered. In particular, it will be called before any calls to
	...
	/**
	 * When smooth scrollbar is enabled, the position and size of the scrollbar thumb is computed based on the number of visible pixels in the
	 * visible items. This however assumes that all list items have the same height. If you use a list in which items have different heights,
	 * the scrollbar will change appearance as the user scrolls through the list. To avoid this issue, you need to disable this property.
	 * 
	 * When smooth scrollbar is disabled, the position and size of the scrollbar thumb is based solely on the number of items in the adapter
	 * and the position of the visible items inside the adapter. This provides a stable scrollbar as the user navigates through a list of
	...
	/**
	 * Indicates whether the children's drawing cache is used during a scroll. By default, the drawing cache is enabled but this will consume
	 * more memory.
	 * 
	...
	/**
	 * Enables or disables the children's drawing cache during a scroll. By default, the drawing cache is enabled but this will use more
	 * memory.
	 * 
	 * When the scrolling cache is enabled, the caches are kept after the first scrolling. You can manually clear the cache by calling
	...
	/**
	 * Get a view and have it show the data associated with the specified position. This is called when we have already discovered that the
	 * view is not available for reuse in the recycle bin. The only choices left are converting an old view or making a new one.
	 * 
	 * @param position
	 *        The position to display
	 * @param isScrap
	 *        Array of at least 1 boolean, the first entry will become true if the returned view was taken from the scrap heap, false if
	 *        otherwise.
	 * 
	...
	/**
	 * Indicates whether this view is in a state where the selector should be drawn. This will happen if we have focus but are not in touch
	 * mode, or we are in the middle of displaying the pressed state for an item.
	 * 
	...
	/**
	 * Creates the ContextMenuInfo returned from {@link #getContextMenuInfo()}. This methods knows the view, position and ID of the item that
	 ...
	 * @return The position of the item which contains the specified point, or {@link #INVALID_POSITION} if the point does not intersect an
	 *         item.
	 */
	 ...
	 * @param y
	 *        Y in local coordinate
	 * @return The rowId of the item which contains the specified point, or {@link #INVALID_ROW_ID} if the point does not intersect an item.
	 */
...
//						 point where they hit the limit first.
	...
	/**
	 * Fires an "on scroll state changed" event to the registered {@link android.widget.AbsListView.OnScrollListener}, if any. The state
	 * change is fired only if the specified state is different from the previously known state.
	 * 
	 * @param newState
	...
	/**
	 * Responsible for fling behavior. Use {@link #start(int)} to initiate a fling. Each frame of the fling is handled in {@link #run()}. A
	...
	private class FlingRunnable implements Runnable {
		...
		void start(int initialVelocity) {
			...
			if (DEBUG)
				Log.d(TAG, String.format("String Fling: [%d, %d] to [%d]", initialY,
						initialVelocity, mScroller.getFinalY()));
	...
	/**
	 * Smoothly scroll to the specified adapter position. The view will scroll such that the indicated position is displayed, but it will stop
	 * early if scrolling further would scroll boundPosition out of view.
	 * @param position
	 *        Scroll to this adapter position.
	 * @param boundPosition
	 ...
	 * 
	 * @param deltaY
	 *        Amount to offset mMotionView. This is the accumulated delta since the motion began. Positive numbers mean the user's finger is
...
//		}
	...
	/**
	 * Returns the number of header views in the list. Header views are special views at the top of the list that should not be recycled
	 * during a layout.
	 * 
	...
	/**
	 * Returns the number of footer views in the list. Footer views are special views at the bottom of the list that should not be recycled
	 * during a layout.
	 * 
	...
	/**
	 * Fills the gap left open by a touch-scroll. During a touch scroll, children that remain on screen are shifted and the other ones are
	 * discarded. The role of this method is to fill the gap thus created by performing a partial layout in the empty space.
	 * 
	 * @param down
	...
	/**
	 * What is the distance between the source and destination rectangles given the direction of focus navigation between them? The direction
	 * basically helps figure out more quickly what is self evident by the relationship between the rects...
	 * 
	 * @param source
	...
	/**
	 * When set to a non-zero value, the cache color hint indicates that this list is always drawn on top of a solid, single-color, opaque
	 * background
	 * 
	 * @param color
	...
	/**
	 * Move all views (excluding headers and footers) held by this AbsListView into the supplied List. This includes views displayed on the
	 * screen as well as views stored in AbsListView's internal view recycler.
	 * 
	 * @param views
	...
	/**
	 * Sets the recycler listener to be notified whenever a View is set aside in the recycler for later reuse. This listener can be used to
	 * free resources associated to the View.
	 * 
	 * @param listener
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		/**
		 * When this boolean is set, the view has been added to the AbsListView at least once. It is used to know whether headers/footers have
		 * already been added to the list view and whether they should be treated as recycled views or not.
		 */
		@ViewDebug.ExportedProperty
		public boolean recycledHeaderFooter;

		/**
		 * When an AbsListView is measured with an AT_MOST measure spec, it needs to obtain children views to measure itself. When doing so,
		 * the children are not attached to the window, but put in the recycler which assumes they've been attached before. Setting this flag
		 * will force the reused view to be attached to the window rather than just attached to the parent.
		 */
		@ViewDebug.ExportedProperty
	...
	/**
	 * A RecyclerListener is used to receive a notification whenever a View is placed inside the RecycleBin's scrap heap. This listener is
	 * used to free resources associated to Views placed in the RecycleBin.
	 * 
	 * @see android.widget.PLA_AbsListView.RecycleBin
	...
	public static interface RecyclerListener {
		/**
		 * Indicates that the specified View was moved into the recycler's scrap heap. The view is not displayed on screen any more and any
	...
	/**
	 * The RecycleBin facilitates reuse of views across layouts. The RecycleBin has two levels of storage: ActiveViews and ScrapViews.
	 * ActiveViews are those views which were onscreen at the start of a layout. By construction, they are displaying current information. At
	 * the end of layout, all views in ActiveViews are demoted to ScrapViews. ScrapViews are old views that could potentially be used by the
	 * adapter to avoid allocating views unnecessarily.
	 * 
	 * @see android.widget.AbsListView#setRecyclerListener(android.widget.AbsListView.RecyclerListener)
	 * @see android.widget.AbsListView.RecyclerListener
	 */
	class RecycleBin {
		...
		/**
		 * Views that were on screen at the start of layout. This array is populated at the start of layout, and at the end of layout all view
		 * in mActiveViews are moved to mScrapViews. Views in mActiveViews represent a contiguous range of Views, with position of the first
		...
		/**
		 * Makes sure that the size of mScrapViews does not exceed the size of mActiveViews. (This can happen if an adapter does not recycle
		 * its views).
		 */
