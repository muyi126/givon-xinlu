...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements
			...
			if (isReadyForPull()) {
				...
				if (absDiff > mTouchSlop
					...
					if (mMode.showHeaderLoadingLayout() && diff >= 1f && isReadyForPullStart()) {
						...
						System.out.println("mIsBeingDragged_1:"+mIsBeingDragged);
						...
						System.out.println("mIsBeingDragged_2:"+mIsBeingDragged);
					}
				}
			}
			break;
	...
	/**
	 * This is implemented by derived classes to return the created View. If you need to use a custom View (such as a custom ListView),
	 ...
	 * @param attrs
	 *        AttributeSet from wrapped class. Means that anything you include in the XML layout declaration will be routed to the created
	 *        View
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS) {
			/**
			 * Use a Hardware Layer on the Refreshable View if we've scrolled at all. We don't use them on the Header/Footer Views as they
			 * change often, which would negate any HW layer performance boost.
			 */
	...
	/**
	 * Updates the View State when the mode has been set. This does not do any checking that the mode is different to current state so always
	 * updates.
	 */
	...
	public static enum AnimationStyle {
		/**
		 * This is the default for Android-PullToRefresh. Allows you to use any drawable, which is automatically rotated and used as a
		 * Progress Bar.
		 */
		ROTATE,

		/**
		 * This is the old default, and what is commonly used on iOS. Uses an arrow image which flips depending on where the user has
		 * scrolled.
		 */
		FLIP;
		...
		/**
		 * Maps an int to a specific mode. This is needed when saving state, or inflating the view from XML where the mode is given through a
		 * attr int.
		 * 
		 * @param modeInt
	...
	public static enum Mode {
		...
		/**
		 * Only allow the user to Pull from the start of the Refreshable View to refresh. The start is either the Top or Left, depending on
		 * the scrolling direction.
		 */
		PULL_FROM_START(0x1),

		/**
		 * Only allow the user to Pull from the end of the Refreshable View to refresh. The start is either the Bottom or Right, depending on
		 * the scrolling direction.
		 */
		PULL_FROM_END(0x2),
	...
	/**
	 * Listener that allows you to be notified when the user has started or finished a touch event. Useful when you want to append extra UI
	 * events (such as sounds). See ( {@link PullToRefreshAdapterViewBase#setOnPullEventListener}.
	 * 
	 * @author Chris Banes
	 */
	public static interface OnPullEventListener<V extends View> {
		 ...
		 * @param direction
		 *        - One of {@link Mode#PULL_FROM_START} or {@link Mode#PULL_FROM_END} depending on which direction the user is pulling. Only
		 *        useful when <var>state</var> is {@link State#PULL_TO_REFRESH} or {@link State#RELEASE_TO_REFRESH}.
		 */
	...
	/**
	 * An advanced version of the Listener to listen for callbacks to Refresh. This listener is different as it allows you to differentiate
	 * between Pull Ups, and Pull Downs.
	 * 
	 * @author Chris Banes
	 */
